...
| ---- | ------------ | ----- |
| 2026-02-17 | domain RGB images for xenium HYP project to upload into samui | NA |
| 2026-02-17 | making domain RGB images for xenium HYP project | NA |
| 2026-02-09 | IF tiles batch stitching groovy script to run in QuPath | NA |
| 2026-02-09 | Qpath in-built stitching pipeline for IF tiles from inform | NA |
| 2026-02-06 | HD QC plots for LS HD data | NA |
| 2026-01-30 | test run NM segmentation on VTA samples | NA |
| 2026-01-13 | Running iLastik software | https://www.ilastik.org/ |
| 2026-01-05 | LIBD_REDcap login issue | NA |
| 2025-11-03 | quick segmentation test on NIA LC HD images using stardist HE model and a custom NM model | NA |
| 2025-11-03 | quick segmentation test on Nac xenium images using stardist HE model | NA |
| 2025-10-30 | spots not aligning with He on samui | fullres HE image was not rotated/transformed like the spots in SPE were, so the mismatch between image and spots happened |
| 2025-10-20 | samui spots not showing up | spot barcodes/keys mitmatch between coords matrix and genes matrix so the spots would show up |
| 2025-09-12 | samui HE image not popping up issue | used unstable version 0.49, should be using 0.45 for proper rendering of the image on website |
| 2025-05-16 | cluster selection from VistoSeg::VNS results | NA |
...
| 2021-11-15 | error in transferring Visuim HPC data to JHPCE | The user directories were cleared during the software updates for LIBD server, emailed Bill to create one for me and this person to use rsync on Neural_Plasiticity. The filenames this person used for 'source' in rync did not exist at the source directory which was throwing errors. |
...
| 2021-02-05 | JHPCE ssh keypair set up | We got this person's macOS laptop registered for passwordless login to JHPCE |
